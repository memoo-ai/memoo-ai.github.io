...
export const columnsOld = (triggerRefresh: Function) => [
  {
    title: '#',
    dataIndex: 'index',
    key: 'index',
...
export const columns = (triggerRefresh: Function) => [
...

  {
    ...
    key: 'tokenAudit',
    sorter: false,
    render: (topTenHoder: number, record: TrendingTokens) => (
      <div className="flex items-center gap-x-[22px]">
        <div className="flex flex-col gap-y-[14px]">
          <p
            className={`font-OCR text-[18px] leading-[20px]  ${Number(topTenHoder) > 0 ? 'text-green' : 'text-[#FE6D6D]'}`}
          >
            {topTenHoder}%
          </p>
        ...
        <div className="flex flex-col gap-y-[14px]">
          <p className="font-OCR text-[18px] leading-[20px] text-green">{record.mintAuthDisable ? 'Yes' : 'No'}</p>
          <h5 className="font-OCR text-[18px] leading-[20px] text-[#7D83B5]">No Mint</h5>
        </div>
        <div className="flex flex-col gap-y-[14px]">
          <p className="font-OCR text-[18px] leading-[20px] text-green">{record.blackList ? 'Yes' : 'No'}</p>
        ...
        <div className="flex flex-col gap-y-[14px]">
          <p className="font-OCR text-[18px] leading-[20px] text-green flex gap-x-[10px]">
            {record.blackList ? 'Yes' : 'No'}
            {record.burntProgress && (
              <IPopover trigger="hover" content={`${record.burntProgress}% Liquidity Burnt`}>
