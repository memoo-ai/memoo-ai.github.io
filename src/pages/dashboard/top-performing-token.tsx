...
const TopPerformingToken: FC = () => {
  ...
  const params = useMemo(() => {
    return [
      {
        key: 'Market Cap',
        value: profileDetail?.marketCap ? profileDetail?.marketCap : '',
      },
      { key: 'ATH Market Cap', value: `$${profileDetail?.athMarketCap ?? 0}` },
      { key: 'Holders', value: profileDetail?.holders ?? 0 },
      { key: 'Holders Growth', value: `${profileDetail?.holdersGrowth ?? 0}` },
    ];
  }, [profileDetail]);
  ...
  return (
    <div className="top-performing-token flex-auto px-5 pt-9 pb-5">
      ...
      <div
        ...
        onClick={() => navigate(`/airdrop/${profileDetail?.ticker}`)}
      >
        <div className="flex items-center  ">
          <img className="w-[67px] h-[67px] rounded-[50%] mr-[7px]" src="./temp/1.png" alt="" />
          <div>
            <h5 className="font-OCR text-[#fff] font-[18px]">Doge Killer </h5>
            <span className="font-OCR text-[#07E993] font-[14px]">LEASH</span>
          </div>
        </div>
